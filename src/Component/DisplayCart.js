 ...
 { dataInfo.map((item) => {
   // console.log(item);
    ...
    return (
      ...
      <div key={item?.items?.card?.info?.id} className="p-6 shadow-2xl bg-zinc-700">
        
        <div className="flex justify-evenly ">

          <div className="w-2/5 m-3">
        <p className="text-3xl">{item?.items?.card?.info?.name}</p>
          <button onClick={() => incrdispatch(increment(item))} className="p-1 bg-red-200 mx-1 rounded-md hover:bg-lime-400">
          <i className="fa-solid fa-plus"></i>
          </button>
            <button onClick={() => dispatch(removeItem(item))}  className="p-1 bg-red-200 mx-1 rounded-lg hover:bg-slate-500"><i className="fa-regular fa-trash-can"></i></button>
          </div>
